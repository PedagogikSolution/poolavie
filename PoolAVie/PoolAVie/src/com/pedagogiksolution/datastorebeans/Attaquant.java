...
public class Attaquant implements Serializable {
	...
	public Attaquant mapAttaquantFromDatastore(Entity mEntity,
			Attaquant mBeanAttaquant) {
	...
	public Entity mapBeanToEntityForDatastore(Attaquant mBeanA, String name) {
		
		Entity mEntity = new Entity("Attaquant", name);

		mEntity.setProperty("acquire_years", mBeanA.getAcquire_years());

		mEntity.setProperty("aide_overtime", mBeanA.getAide_overtime());

		mEntity.setProperty("birthday", mBeanA.getBirthday());

		mEntity.setProperty("blanchissage", mBeanA.getBlanchissage());

		mEntity.setProperty("but_victoire", mBeanA.getBut_victoire());

		mEntity.setProperty("can_be_rookie", mBeanA.getCan_be_rookie());

		mEntity.setProperty("club_ecole", mBeanA.getClub_ecole());

		mEntity.setProperty("contrat", mBeanA.getContrat());

		mEntity.setProperty("nom", mBeanA.getNom());

		mEntity.setProperty("pj", mBeanA.getPj());

		mEntity.setProperty("players_id", mBeanA.getPlayers_id());

		mEntity.setProperty("position", mBeanA.getPosition());

		mEntity.setProperty("projection", mBeanA.getProjection());

		mEntity.setProperty("pts", mBeanA.getPts());

		mEntity.setProperty("salaire_draft", mBeanA.getSalaire_draft());

		mEntity.setProperty("team_id", mBeanA.getTeam_id());

		mEntity.setProperty("teamOfPlayer", mBeanA.getTeamOfPlayer());

		mEntity.setProperty("years_1", mBeanA.getYears_1());

		mEntity.setProperty("years_2", mBeanA.getYears_2());

		mEntity.setProperty("years_3", mBeanA.getYears_3());

		mEntity.setProperty("years_4", mBeanA.getYears_4());

		mEntity.setProperty("years_5", mBeanA.getYears_5());
