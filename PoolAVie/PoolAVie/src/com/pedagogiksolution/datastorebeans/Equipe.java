...
public class Equipe implements Serializable {
	...
	public Entity mapBeanToEntityForDatastore(Equipe mBean, String name) {

		Entity mEntity = new Entity("Equipe", name);

		mEntity.setProperty("argent_recu", mBean.getArgent_recu());

		mEntity.setProperty("bonus_5m", mBean.getBonus_5m());

		mEntity.setProperty("bonus_penalite", mBean.getBonus_penalite());

		mEntity.setProperty("budget_restant", mBean.getBudget_restant());

		mEntity.setProperty("classement_last_year", mBean.getClassement_last_year());

		mEntity.setProperty("manquant_att", mBean.getManquant_att());

		mEntity.setProperty("manquant_def", mBean.getManquant_def());

		mEntity.setProperty("manquant_gardien", mBean.getManquant_gardien());

		mEntity.setProperty("manquant_recrue", mBean.getManquant_recrue());

		mEntity.setProperty("manquant_equipe", mBean.getManquant_equipe());

		mEntity.setProperty("max_salaire_begin", mBean.getMax_salaire_begin());

		mEntity.setProperty("moy_sal_restant_draft", mBean.getMoy_sal_restant_draft());

		mEntity.setProperty("nb_attaquant", mBean.getNb_attaquant());
		
		mEntity.setProperty("nb_defenseur", mBean.getNb_defenseur());

		mEntity.setProperty("nb_gardien", mBean.getNb_gardien());

		mEntity.setProperty("nb_rookie", mBean.getNb_rookie());

		mEntity.setProperty("nb_equipe", mBean.getNb_equipe());

		mEntity.setProperty("nb_contrat", mBean.getNb_contrat());
