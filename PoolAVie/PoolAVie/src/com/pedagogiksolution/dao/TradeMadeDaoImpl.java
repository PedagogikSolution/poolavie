...
public class TradeMadeDaoImpl implements TradeMadeDao {
	...
	private static final String REMOVE_PRIMARY_KEY = "ALTER TABLE trade_made_archive_? CHANGE COLUMN `_id` `_id` INT(11) NULL ,DROP PRIMARY KEY";
	...
	@Override
	public void insertTradeMade(int poolId, int trade_id, int cycleAnnuel) throws DAOException {
		Connection connexion = null;
		PreparedStatement preparedStatement = null;

		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, INSERT_TRADE_OFFER_INTO_TRADE_MADE, false,
					poolId, poolId, trade_id);
	...
	public int getNumberTradeMadeByMe(String poolID, int teamId) throws DAOException {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, GET_NUMBER_TRADE_MADE_BY_ME, false,
					Integer.parseInt(poolID), teamId, teamId);
	...
	public TradeBeans getTradeMadeByMe(Pool mBeanPool, String poolID, int teamId, int i) throws DAOException {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, GET_TRADE_MADE_BY_ME, false,
					Integer.parseInt(poolID), teamId, teamId);
			rs = preparedStatement.executeQuery();
			while (rs.next()) {
	...
	public int getNumberTradeMadeByAll(String poolID) throws DAOException {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, GET_NUMBER_TRADE_MADE_BY_ALL, false,
	...
	public TradeBeans getTradeMadeByAll(Pool mBeanPool, String poolID, int teamId, int i) throws DAOException {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, GET_TRADE_MADE_BY_ALL, false,
					Integer.parseInt(poolID));
			rs = preparedStatement.executeQuery();
			while (rs.next()) {
				trade_id.add(rs.getInt("_id"));
	...
	@Override
	public TradeBeans showOfferX(Pool mBeanPool, Utilisateur mBeanUser, int trade_id, PlayersDao playersDao,
			DraftPickDao draftPickDao, DraftDao draftDao) {
		Connection connexion = null;
		ResultSet rs = null;
		PreparedStatement preparedStatement = null;
		TradeBeans mBean = new TradeBeans();
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, SHOW_TRADE_Y, false,
					Integer.parseInt(mBeanPool.getPoolID()), trade_id);
			rs = preparedStatement.executeQuery();
			while (rs.next()) {
		...
		draftPickTeamThatReceived = draftPickTeamThatReceivedTemp
				.toArray(new String[draftPickTeamThatReceivedTemp.size()]);
		...
		int budgetReceivingOffer = (total_salaire_team_receiving_offer - argentOfferTeamThatReceivedOffer)
				- (total_salaire_team_making_offer - argentOfferTeamThatTrade);
	...
	public void insertionDansArchives(HttpServletRequest req) {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, ARCHIVE_TRADE_LAST_YEAR, false,
	...
	public void resetTradeMade(String poolID) {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connexion, TRUNCATE_FOR_NEW_YEARS, false,
					Integer.parseInt(poolID));
			preparedStatement.execute();
