			...
			if (conn != null) {
				...
				try {
					...
					if (sortBy.equalsIgnoreCase("pts")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='gardien' ORDER BY pts DESC";
					} else if (sortBy.equalsIgnoreCase("proj")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='gardien' ORDER BY projection DESC";
					} else if (sortBy.equalsIgnoreCase("salaire")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='gardien' ORDER BY salaire_draft DESC";
					} else {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='gardien'";
					}
				...
				try {
					...
					req.getRequestDispatcher("/jsp/gardien_draft.jsp").forward(
							req, resp);					
