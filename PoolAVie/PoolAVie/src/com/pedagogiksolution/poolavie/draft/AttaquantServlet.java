...
public class AttaquantServlet extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = -9126048396062272819L;
	String statement,statement5;
	Connection conn;
	ResultSet rs,rs5;
			...
			if (conn != null) {
				...
				try {
					...
					if (sortBy.equalsIgnoreCase("pts")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='attaquant' ORDER BY pts DESC";
					} else if (sortBy.equalsIgnoreCase("proj")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='attaquant' ORDER BY projection DESC";
					} else if (sortBy.equalsIgnoreCase("salaire")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='attaquant' ORDER BY salaire_draft DESC";
					} else {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='attaquant'";
					}
				...
				try {
					...
					req.getRequestDispatcher("/jsp/attaquant_draft.jsp").forward(
							req, resp);					
