...
public class DefenseurServlet extends HttpServlet {
	/**
	 * 
	 */
			...
			if (conn != null) {
				...
				try {
					...
					if (sortBy.equalsIgnoreCase("pts")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='defenseur' ORDER BY pts DESC";
					} else if (sortBy.equalsIgnoreCase("proj")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='defenseur' ORDER BY projection DESC";
					} else if (sortBy.equalsIgnoreCase("salaire")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='defenseur' ORDER BY salaire_draft DESC";
					} else {
						statement = "SELECT * FROM players WHERE contrat=0 AND position='defenseur'";
					}
				...
				try {
					...
					req.getRequestDispatcher("/jsp/defenseur_draft.jsp").forward(
							req, resp);					
