...
public class RecrueServlet extends HttpServlet {
	/**
	 * 
	 */
			...
			if (conn != null) {
				...
				try {
					...
					if (sortBy.equalsIgnoreCase("pts")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND can_be_rookie=1 ORDER BY pts DESC";
					} else if (sortBy.equalsIgnoreCase("proj")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND can_be_rookie=1 ORDER BY projection DESC";
					} else if (sortBy.equalsIgnoreCase("salaire")) {
						statement = "SELECT * FROM players WHERE contrat=0 AND can_be_rookie=1 ORDER BY salaire_draft DESC";
					} else {
						statement = "SELECT * FROM players WHERE contrat=0 AND can_be_rookie=1";
					}
				...
				try {
					...
					req.getRequestDispatcher("/jsp/recrue_draft.jsp").forward(
							req, resp);					
