...
public class TradeModel {
	...
	public Boolean getDataOnTeamThatTrade() {
		...
		DraftPick mBeanDraftPickThatMakingOffer = new DraftPick();
		DraftPick mBeanDraftPickThatReceivedOffer = new DraftPick();
		...
		if (cycleAnnuel == 5 || cycleAnnuel == 6 || cycleAnnuel == 11) {
			...
			mBeanDraftPickThatReceivedOffer = (DraftPick) req.getSession()
					.getAttribute(draftPickNameTeamReceivingOffer);
		}
		...
		Attaquant mBeanAttaquantThatMakingOffer = (Attaquant) req.getSession()
				.getAttribute(attaquantNameTeamMakingOffer);
		NonSessionAttaquant mNonSessionAttaquantBeanThatMakingOffer = new NonSessionAttaquant();

		Attaquant mBeanAttaquantThatReceivedOffer = (Attaquant) req.getSession()
				.getAttribute(attaquantNameTeamReceivingOffer);
		NonSessionAttaquant mNonSessionAttaquantBeanThatReceivedOffer = new NonSessionAttaquant();

		Defenseur mBeanDefenseurThatMakingOffer = (Defenseur) req.getSession()
				.getAttribute(defenseurNameTeamMakingOffer);
		NonSessionDefenseur mNonSessionDefenseurBeanThatMakingOffer = new NonSessionDefenseur();

		Defenseur mBeanDefenseurThatReceivedOffer = (Defenseur) req.getSession()
				.getAttribute(defenseurNameTeamReceivingOffer);
		...
		if (cycleAnnuel == 5 || cycleAnnuel == 6 || cycleAnnuel == 11) {
			...
			mNonSessionDraftBeanThatMakingOffer
					.setTeamNameOriginalPick(mBeanDraftPickThatMakingOffer.getTeamNameOriginalPick());
			...
			mNonSessionDraftBeanThatReceivedOffer
					.setTeamNameOriginalPick(mBeanDraftPickThatReceivedOffer.getTeamNameOriginalPick());
		...
		if (cycleAnnuel == 3) {
			...
			// on va chercher le bean DraftRound dans la session
			List<Long> mTeamId = new ArrayList<Long>();
			mTeamId = mBeanDraftRound.getTeam_id();
			List<Long> mRonde = new ArrayList<Long>();
			mRonde = mBeanDraftRound.getRonde();
			List<Long> mPickNo = new ArrayList<Long>();
			mPickNo = mBeanDraftRound.getDraft_pick_no();
			List<String> mTeamName = new ArrayList<String>();
			mTeamName = mBeanDraftRound.getFrom_who();

			// on va chercher le currentPick
			...
			try {
				...
				currentPick = currentPickL.intValue();
			} catch (EntityNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}

			// on init des Array vide
			...
			for (int i = 1; i < (currentPick + 1); i++) {
				mIterator.next();
			}
			while (mIterator.hasNext()) {
				int indexListIterator = mIterator.nextIndex();
				Long teamIdIterator = mIterator.next();
				Long teamNumber = mTeamId.get(indexListIterator);
				if (teamNumber.intValue() == teamThatMakeOfferId) {
			...
			ListIterator<Long> mIterator2 = mTeamId.listIterator();
			for (int i = 1; i < (currentPick + 1); i++) {
				mIterator2.next();
			}
			while (mIterator2.hasNext()) {
				int indexListIterator2 = mIterator2.nextIndex();
				Long teamIdIterator2 = mIterator2.next();
				Long teamNumber2 = mTeamId.get(indexListIterator2);
				if (teamNumber2.intValue() == teamThatReceivedOfferId) {
					Long draft_pick_no2 = mPickNo.get(indexListIterator2);
					Long mRondeId2 = mRonde.get(indexListIterator2);
					ronde2.add(mRondeId2);
					String mName = mTeamName.get(indexListIterator2);
					teamNameOriginePick2.add(mName);
					team_id2.add(teamIdIterator2);
					pick_no2.add(draft_pick_no2);
	...
	public Boolean checkIfTradeIsValid(PlayersDao playersDao, DraftPickDao draftPickDao, DraftDao draftDao) {

		// retourne false si trade pas possible // retourne true if trade is good to go
		// at this point
		...
		if ((nbPlayersTeamMakingOffer == 0 && nbPicksTeamMakingOffer == 0 && cashIncludeTeamThatMakeOfferInt == 0
				&& nbRookieTeamMakingOffer == 0)
		...
		// on calcul le total d'Argent des salaire des joueurs de l'Equipe qui trade, le
		// nombre par position inclus dans
		// trade
		// et on ajoute leur nom dans un array

		int i = 0;
		int nbJoueurSousContratRecu = 0;
		int nbJoueurSousContratDonne = 0;
		if (playersIdTeamThatMakeOffer != null) {
			...
			for (String s : playersIdTeamThatMakeOffer) {
				...
				total_salaire_team_making_offer = total_salaire_team_making_offer
				...
				if (mBeanTemp.getNomMakingOfferString() != null) {
					...
					if (mBeanTemp.getYears_2().equalsIgnoreCase("A") || mBeanTemp.getYears_2().equalsIgnoreCase("JA")
						...
						nbJoueurSousContratDonne = nbJoueurSousContratDonne + 1;
					}
		...
		// on calcul le total d'Argent des salaire des joueurs de l'Equipe qui recoit et
		// on ajoute leur nom dans un
		// array
		int j = 0;
		if (playersIdTeamThatReceiveOffer != null) {
			...
			for (String s : playersIdTeamThatReceiveOffer) {
				...
				if (mBeanTemp.getYears_2().equalsIgnoreCase("A") || mBeanTemp.getYears_2().equalsIgnoreCase("JA")
					...
					nbJoueurSousContratRecu = nbJoueurSousContratRecu + 1;
				}
		...
		if (picksIdTeamThatMakeOffer != null) {
			...
			for (String s : picksIdTeamThatMakeOffer) {
				...
				if (mBeanPool.getCycleAnnuel() == 3) {
					mBeanTemp = draftDao.getRoundAndNameOfTeam(poolID, toInt, mBeanPool);
				} else {
					mBeanTemp = draftPickDao.getNameOfTeam(poolID, toInt, mBeanPool);
				}
		...
		int budgetMakingOffer = (total_salaire_team_making_offer - cashIncludeTeamThatMakeOfferInt)
				- (total_salaire_team_receiving_offer - cashIncludeThatReceiveOfferInt);
		int budgetReceivingOffer = (total_salaire_team_receiving_offer - cashIncludeThatReceiveOfferInt)
				- (total_salaire_team_making_offer - cashIncludeTeamThatMakeOfferInt);
	...
	public void persistTradeOffer(HttpServletRequest req, TradeOfferDao tradeOfferDao) {
		...
		String t1j1 = null, t1j2 = null, t1j3 = null, t1j4 = null, t1j5 = null, t1j6 = null, t1j7 = null, t2j1 = null,
				t2j2 = null, t2j3 = null, t2j4 = null, t2j5 = null, t2j6 = null, t2j7 = null, t1p1 = null, t1p2 = null,
				t1p3 = null, t2p1 = null, t2p2 = null, t2p3 = null;
		...
		tradeOfferDao.insertTradeOffer(poolID, teamMakingOfferId, teamReceivingOfferId, t1j1, t1j2, t1j3, t1j4, t1j5,
				t1j6, t1j7, t2j1, t2j2, t2j3, t2j4, t2j5, t2j6, t2j7, t1p1, t1p2, t1p3, t2p1, t2p2, t2p3,
				cashMakingOffer, cashReceivingOffer, mBeanPool.getCycleAnnuel(), annee, messageOffre);
	...
	public void showOfferNumberX(int from, TradeOfferDao tradeOfferDao, TradeMadeDao tradeMadeDao,
			PlayersDao playersDao, DraftPickDao draftPickDao, DraftDao draftDao) {
	...
	public Boolean checkIfTradeIsStillPossible(PlayersDao playersDao, DraftPickDao draftPickDao,
			TradeOfferDao tradeOfferDao, DraftDao draftDao) {
		// on verifie d'Abord si tout les élement du trade son encore dans l'équipe
		...
		boolean checkIfTradeIsStillValidateByRule = checkIfTradeIsStillValidateByRule(playersDao, req,
				mBeanTemp.getTeam_1(), mBeanTemp.getTeam_2(), playersTeamThatOffer, playersTeamThatReceived,
				mBeanTemp.getT1_cash(), mBeanTemp.getT2_cash());
	...
	private boolean checkIfTradeIsStillValidateByRule(PlayersDao playersDao, HttpServletRequest req, String team_1,
			String team_2, String[] playersTeamThatOffer, String[] playersTeamThatReceived, int t1_cash, int t2_cash) {
		...
		if (playersIdTeamThatMakeOffer != null) {
			...
			for (String s : playersIdTeamThatMakeOffer) {
				...
				TradeBeanTemp mBeanTemp = playersDao.getPlayersById(poolID, toInt, 2);

				if (mBeanTemp.getClubEcole() == 1) {
					nbRookieTeamMakingOffer = nbRookieTeamMakingOffer + 1;
				} else {
		...
		if (playersIdTeamThatReceiveOffer != null) {
			...
			for (String s : playersIdTeamThatReceiveOffer) {
				...
				if (mBeanTemp.getClubEcole() == 1) {
					nbRookieTeamReceivingOffer = nbRookieTeamReceivingOffer + 1;
				} else {
					total_salaire_team_receiving_offer = total_salaire_team_receiving_offer
							+ mBeanTemp.getTotal_salaire_team_making_offer();
	...
	public void makeTrade(PlayersDao playersDao, DraftPickDao draftPickDao, TradeOfferDao tradeOfferDao,
		...
		if (mBeanTemp.getT1j2() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_1(), mBeanTemp.getTeam_2(), mBeanTemp.getT1j2(),
					playersDao);
		}
		if (mBeanTemp.getT1j3() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_1(), mBeanTemp.getTeam_2(), mBeanTemp.getT1j3(),
					playersDao);
		}
		if (mBeanTemp.getT1j4() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_1(), mBeanTemp.getTeam_2(), mBeanTemp.getT1j4(),
					playersDao);
		}
		if (mBeanTemp.getT1j5() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_1(), mBeanTemp.getTeam_2(), mBeanTemp.getT1j5(),
					playersDao);
		}
		if (mBeanTemp.getT1j6() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_1(), mBeanTemp.getTeam_2(), mBeanTemp.getT1j6(),
					playersDao);
		}
		if (mBeanTemp.getT1j7() != null) {
		...
		if (mBeanTemp.getT2j2() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_2(), mBeanTemp.getTeam_1(), mBeanTemp.getT2j2(),
					playersDao);
		}
		if (mBeanTemp.getT2j3() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_2(), mBeanTemp.getTeam_1(), mBeanTemp.getT2j3(),
					playersDao);
		}
		if (mBeanTemp.getT2j4() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_2(), mBeanTemp.getTeam_1(), mBeanTemp.getT2j4(),
					playersDao);
		}
		if (mBeanTemp.getT2j5() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_2(), mBeanTemp.getTeam_1(), mBeanTemp.getT2j5(),
					playersDao);
		}
		if (mBeanTemp.getT2j6() != null) {
			movePlayersFromFirstToSecondTeam(mBeanTemp.getTeam_2(), mBeanTemp.getTeam_1(), mBeanTemp.getT2j6(),
					playersDao);
		}
		if (mBeanTemp.getT2j7() != null) {
	...
	private Boolean checkIfPickStillInTeam(String team, String identifiant, DraftPickDao draftPickDao,
			DraftDao draftDao, Pool mBeanPool2) {
		int teamId = Integer.parseInt(team);
		int pickId = Integer.parseInt(identifiant);
		String poolID = mBeanPool.getPoolID();
		int poolId = Integer.parseInt(poolID);
		Boolean checkIfPicksStillInTeam = false;
		if (mBeanPool.getCycleAnnuel() == 3) {
			checkIfPicksStillInTeam = draftDao.checkIfPickStillInTeam(poolId, teamId, pickId);
		} else {
			checkIfPicksStillInTeam = draftPickDao.checkIfPicksStillInTeam(poolId, teamId, pickId);
		}

		return checkIfPicksStillInTeam;
	}

	public void putDatabaseInDatastore(ClassementDao classementDao, PlayersDao playersDao, DraftPickDao draftPickDao,
			HttpServletRequest req) {
