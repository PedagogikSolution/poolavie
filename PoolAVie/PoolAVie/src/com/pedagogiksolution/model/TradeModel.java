...
public class TradeModel {
	...
	if (teamThatReceivedOfferId == teamThatMakeOfferId) {
	    ...
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire un �change avec vous-m�me... veuillez choisir une autre �quipe");
	...
	if (nbPlayersTeamMakingOffer > 7 || nbPlayersTeamReceivingOffer > 7) {
	    ...
	    mBeanMessageErreur.setErreurTrade("Un maximum de 7 joueurs par �quipe peut �tre inclus dans un �change unique (Reglement 4.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}
	...
	if (nbPicksTeamMakingOffer > 3 || nbPicksTeamReceivingOffer > 3) {
	    mBeanMessageErreur.setErreurTrade("Un maximum de 3 choix au rep�chage par �quipe peut �tre inclus dans un �change unique (Reglement 4.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}
	...
	if (cashIncludeTeamThatMakeOfferInt > 0 && cashIncludeThatReceiveOfferInt > 0) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas �changez de l'argent contre de l'argent (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}
	...
	if (mBeanPool.getCycleAnnuel() == 5 || mBeanPool.getCycleAnnuel() == 6) {
		...
		mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas �changer un choix contre de l'argent � cette p�riode (Reglement 3.1");
		req.setAttribute("messageErreur", mBeanMessageErreur);
		return false;
...
// array
	...
	if ((nb_attaquant_make_offer - nbAttInTeamThatOffer + nbAttInTeamThatReceived) < 8) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire cette �change sous peine de vous retrouver avec moins de 8 attaquants (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}

	if ((nb_attaquant_rec_offer + nbAttInTeamThatOffer - nbAttInTeamThatReceived) < 8) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire cette �change sous peine que la personne avec qui vous �changez se retrouve avec moins de 8 attaquants (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}

	if ((nb_defenseur_make_offer - nbDefInTeamThatOffer + nbDefInTeamThatReceived) < 5) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire cette �change sous peine de vous retrouver avec moins de 5 defenseurs (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}

	if ((nb_defenseur_rec_offer + nbDefInTeamThatOffer - nbDefInTeamThatReceived) < 5) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire cette �change sous peine que la personne avec qui vous �changez se retrouve avec moins de 5 defenseurs (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}

	if ((nb_goaler_make_offer - nbGoalInTeamThatOffer + nbGoalInTeamThatReceived) < 2) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire cette �change sous peine de vous retrouver avec moins de 2 gardiens (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}

	if ((nb_goaler_rec_offer + nbGoalInTeamThatOffer - nbGoalInTeamThatReceived) < 2) {
	    mBeanMessageErreur.setErreurTrade("Vous ne pouvez pas faire cette �change sous peine que la personne avec qui vous �changez se retrouve avec moins de 2 gardiens (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	}
	...
	if ((budget_restant_make_offer + total_salaire_team_making_offer - total_salaire_team_receiving_offer - argent_recu_make_offer + argent_recu_rec_offer + cashIncludeTeamThatMakeOfferInt - cashIncludeThatReceiveOfferInt) < 0) {
	    ...
	    mBeanMessageErreur.setErreurTrade("Vous n'avez pas assez d'argent pour effectuer cette �change (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	...
	if ((budget_restant_received_offer - total_salaire_team_making_offer + total_salaire_team_receiving_offer + argent_recu_make_offer - argent_recu_rec_offer + cashIncludeThatReceiveOfferInt - cashIncludeTeamThatMakeOfferInt) < 0) {
	    ...
	    mBeanMessageErreur.setErreurTrade("La personne avec qui vous voulez �changez n'a pas le budget pour absorber cette transaction (Reglement 3.1");
	    req.setAttribute("messageErreur", mBeanMessageErreur);
	    return false;
	...
	try {
	    ...
	    msg.setContent("Bonjour " + nomTeam + ", \n\n Vous avez re�u une nouvelle offre d'�change. Pour voir celle-ci, l'accepter, la refuser ou faire une contre offre. Connectez-vous et aller dans la section Trade Center.", "text/html");
	    Transport.send(msg);
