...
<body>
	...
	<div class="main_container">
		<div class="main_content">
			<div id="main_content_title_classement">SECTION DES OFFRES QUE
				J'AI RECU</div>
			...
			<div id="main_content_table_classement">
				...
				<c:if test="${messageErreur.codeErreurOffreTrade==11}">
					<h1 style="color: red">
						<strong>Un ou des joueurs ou un ou des pick ne sont plus
							disponible dans l'une des équipes ou bien l'une des deux équipes
							n'a plus le budget disponible pour faire le trade</strong>
					</h1>
				</c:if>
			...
			Tu n'as aucune offre pour le moment. Trouve toi des amis 
				...
				<c:if test="${beanTrade.isThereAOfferForMe==1}">
					...
					<br>
					<c:set var="longueur"
						value="${beanAffichageOfferMade.tradeOfferId}" />
					<table>
						<tr>
							<th>offre #</th>
							<th>équipe</th>
								...
								<tr>
									...
									<td>
										<form action="/trade_received" method="post">
											<input type="hidden"
												value="${beanAffichageOfferMade.tradeOfferId[i]}"
												name="trade_id" /> <input type="hidden" value="showOffer"
												name="etape" /> <input style="font-size: 18px"
									...
									<td>
										<form action="/trade_received" method="post">
											<input type="hidden"
												value="${beanAffichageOfferMade.tradeOfferId[i]}"
												name="trade_id" /> <input type="hidden" value="cancelOffer"
												name="etape" /> <input style="font-size: 18px"
									...
									<td>
										<form action="/trade_received" method="post">
											<input type="hidden"
												value="${beanAffichageOfferMade.tradeOfferId[i]}"
												name="trade_id" /> <input type="hidden" value="acceptOffer"
												name="etape" /> <input style="font-size: 18px"
												type="submit" value="Accepter cette offre">
										</form>
									</td>
